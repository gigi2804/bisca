...
console.log("SERVER AVVIATO: Versione Finale (Fix Asso Cieco)");
...
io.on('connection', (socket) => {
  ...
  socket.on('playCard', (data) => { 
      try { 
          ...
          // --- NUOVO: SE ASSO DI DENARI E NESSUNA SCELTA, FERMATI E CHIEDI ---
          if (c.suit === 'denari' && c.value === 1 && !data.assoChoice) {
              socket.emit('askAsso', data.cardIndex);
              return; 
          }
          // -------------------------------------------------------------------
